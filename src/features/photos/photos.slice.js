...
const options = {
  ...
  reducers: {
    ...
    removePhoto: (state, action) => {
      ...
      state.photos = state.photos.filter((x) => x.id !== action.payload);
    },
  },
};
...
export const selectFilteredPhotos = (state) => {
  ...
  return state.photos.photos.filter((x) =>
    x.caption.toLowerCase().includes(state.search.searchTerm.toLowerCase())
  );
};
